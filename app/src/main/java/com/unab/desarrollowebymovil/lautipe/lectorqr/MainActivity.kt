    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (result != null) {
            if(result.contents == null){
                ...
                val scannedId = result.contents.toIntOrNull()
                if (scannedId != null) {
                    val api = ApiClient.getClient().create(ApiInterface::class.java)
                    ...
                    call.enqueue(object : Callback<PaymentInfo> {
                        ...
                        override fun onFailure(call: Call<PaymentInfo>, t: Throwable) {
                            Toast.makeText(this@MainActivity, "Error al obtener información del pago", Toast.LENGTH_SHORT).show()
                        }
                    })
                } else {
                    Toast.makeText(this, "El valor escaneado no es válido", Toast.LENGTH_SHORT).show()
                }
            }
        } else {
